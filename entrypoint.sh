...
load_state() {
  ...
  # padding приводит к тому, что openssl молча декодирует неполный результат.
...
load_profile() {
  ...
  # Разбор такой же, как в load_state: сохраняем хвостовой '=' base64.
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in *=*) ;; *) continue ;; esac
    key=${line%%=*}
    value=${line#*=}
...
# Remnawave умеет отдавать подписку зашифрованной (Response Rules ->
# responseModifications.encryption). Тело приходит как age в ASCII-armor, а
# приватный ключ хранится в профиле. Расшифровка делается ядром: mihomo несёт
# в себе age и умеет "mihomo age decrypt <secret_key> <in> <out>".
#
# Ключ передаётся аргументом, потому что подкоманда age не читает его из
# окружения: она отрабатывает раньше разбора флагов, до SetGlobalSecretKeys.
# Внутри контейнера это единственный процесс от root, но иметь в виду стоит.
AGE_ARMOR_HEADER='-----BEGIN AGE ENCRYPTED FILE-----'

source_is_encrypted() {
  [ -s "$1" ] || return 1
  read -r decrypt_first_line < "$1" || return 1
  case "$decrypt_first_line" in "$AGE_ARMOR_HEADER"*) return 0 ;; esac
  return 1
}

# Возвращает 0, если файл готов к дальнейшей обработке (расшифрован или и не
# был зашифрован). При ошибке заполняет FETCH_ERROR.
decrypt_source_file() {
  decrypt_target="$1"
  if ! source_is_encrypted "$decrypt_target"; then
    if [ -n "$(b64_decode_file "$SUB_AGE_KEY_B64")" ]; then
      log "subscription is not encrypted, age key is unused"
    fi
    return 0
  fi
  decrypt_key="$(b64_decode_file "$SUB_AGE_KEY_B64")"
  if [ -z "$decrypt_key" ]; then
    FETCH_ERROR="Подписка зашифрована (age), но ключ расшифровки не задан в профиле"
    return 1
  fi
  decrypt_out="$decrypt_target.plain"
  decrypt_log="$RUNTIME_DIR/age-decrypt.$$.log"
  rm -f "$decrypt_out"
  if ! mihomo age decrypt "$decrypt_key" "$decrypt_target" "$decrypt_out" > "$decrypt_log" 2>&1; then
    # Подкоманда age завершается через panic(), поэтому причина лежит в первой
    # строке вида "panic: ...", а всё остальное — стек горутины.
    decrypt_detail=$(sed -n 's/^panic: //p' "$decrypt_log" 2>/dev/null | head -n 1 | head -c 200)
    FETCH_ERROR="Не удалось расшифровать подписку: проверьте, что ключ соответствует публичному ключу в правиле панели"
    [ -z "$decrypt_detail" ] || FETCH_ERROR="$FETCH_ERROR ($decrypt_detail)"
    rm -f "$decrypt_out" "$decrypt_log"
    return 1
  fi
  rm -f "$decrypt_log"
  if [ ! -s "$decrypt_out" ]; then
    FETCH_ERROR="Расшифрованная подписка пуста"
    rm -f "$decrypt_out"
    return 1
  fi
  mv "$decrypt_out" "$decrypt_target" || {
    FETCH_ERROR="Расшифрованную подписку не удалось сохранить"
    rm -f "$decrypt_out"
    return 1
  }
  FETCH_DECRYPTED=1
...
EOF
  ...
  # Расшифровываем до сохранения: дальше по конвейеру (оверрайды, listeners,
  # mihomo -t, просмотр YAML в вебке) везде ожидается открытый текст.
  if ! decrypt_source_file "$tmp"; then
