...
![Веб-панель RemnaSub RoS](/docs/screenshots/subscriptions.png)
...
![Настройки — переопределения](/docs/screenshots/settings-overrides.png)
...
- **Переопределения**: режим маршрутизации, управляемые параметры Mihomo, sniffer и общий YAML для всех подписок.
- **Оформление**: тёмная и светлая темы, пресеты «Графит», «Полночь», «Тайга», «Сепия», плюс свой
  акцентный цвет. Выбор хранится в контейнере и действует для всех, кто открывает панель.
- **Доступ**: генератор хеша для `BASIC_AUTH_HASH`.

![Настройки — оформление](/docs/screenshots/settings-appearance.png)

Настройки конкретной подписки открываются из её строки: URL, интервал, таймаут, ключ расшифровки и локальные переопределения.

![Настройки подписки](/docs/screenshots/subscription-editor.png)

Итоговый конфиг и журнал контейнера доступны прямо из панели.

![Рабочий YAML](/docs/screenshots/runtime-yaml.png)
...
Значения этих пяти ключей можно изменить, а сам заголовок — выключить галочкой слева. Выключенный заголовок не отправляется вообще: для обязательного ключа это означает, что подстановки значения по умолчанию тоже не будет. Значение при этом сохраняется, чтобы вернуть его одной галочкой — в настройках такая строка хранится с решёткой впереди.

Разрешено добавлять любые собственные заголовки, их тоже можно выключать. Имена сравниваются без учёта регистра.
...
- `mode` берётся из подписки, пока в настройках не выбран конкретный режим (`rule`, `global`, `direct`);
- общий YAML из настроек и локальный YAML профиля заменяют одноимённые верхнеуровневые секции до применения обязательных параметров контейнера.

Приоритет: **исходный YAML → общий YAML → локальный YAML профиля → управляемые глобальные параметры → локальные listeners и controller**.

### Общий и локальный YAML

Общий YAML задаётся один раз в **Настройки → Переопределения** и накладывается на каждую подписку. Локальный YAML профиля идёт следом, поэтому подписка может отменить общее правило для своей секции.

Наложение работает **заменой верхнеуровневой секции целиком, а не слиянием**. Если переопределить `dns`, из исходной секции не останется ничего, кроме написанного вами, — указывайте блок полностью. Кнопки-пресеты над редактором вставляют готовые секции (DoH-резолверы, пиннинг их адресов, доступ из LAN, геоданные, тюнинг соединений).

Два ограничения формата проверяются до сохранения, потому что ломают разбор:

- список на верхнем уровне (`- MATCH,DIRECT` без ключа над ним) — элементы должны быть с отступом под своей секцией, иначе строка вида `- IP-CIDR,2001:db8::/32,PROXY` неотличима от начала нового ключа;
- табы в отступах — YAML их не допускает.

Ключи, которые контейнер выставляет сам уже после наложения, в YAML писать бесполезно: `find-process-mode`, `log-level`, `ipv6`, `profile`, `listeners`, `redir-port`, `tproxy-port`, `tun`, `external-controller*`, `external-ui*`, `secret`. Редактор подсвечивает их прямо при вводе.

## 🔒 Шифрованные подписки

Remnawave умеет отдавать подписку зашифрованной — в правиле ответов (Subscription Response Rules) задаётся `responseModifications.encryption`:
...
Тело ответа приходит как [age](https://age-encryption.org) в ASCII-armor, начинаясь с `-----BEGIN AGE ENCRYPTED FILE-----`. Поддерживаются оба метода: `age1` (X25519) и `age1pq1` (постквантовый гибрид ML-KEM-768 + X25519).

Контейнер расшифровывает подписку сразу после загрузки, до применения переопределений и проверки `mihomo -t`, поэтому дальше всё работает как с обычным YAML — включая просмотр **Полученный YAML** в веб-панели. Расшифровка выполняется самим ядром: mihomo несёт в себе age.

**Настройка:**

1. Откройте карточку подписки → блок **Шифрование ответа**.
2. Нажмите **Сгенерировать**, выбрав метод. Пара создаётся внутри контейнера: приватный ключ сразу попадает в поле и никуда не отправляется, публичный показывается для копирования.
3. Вставьте публичный ключ в `encryption.key` правила ответов панели, а метод укажите тот же.
4. Сохраните подписку — она будет перезагружена уже зашифрованной.

Если пара уже сгенерирована в панели (`docker exec -it remnawave cli` → *Generate keypairs*), просто вставьте её приватную часть (`AGE-SECRET-KEY-1...` или `AGE-SECRET-KEY-PQ-1...`) в поле ключа.

Поле пустое — подписка считается незашифрованной. Если сервер всё же прислал зашифрованное тело, а ключ не задан, в карточке появится понятная ошибка вместо непрозрачного сбоя разбора YAML.
...
### Входы из подписки и свои прокси

Remnawave присылает в конфиге собственные входы — обычно `mixed-port: 7890`, `socks-port: 7891` и `port`. Контейнер работает шлюзом, и лишний слушающий порт в LAN ему не нужен, поэтому **по умолчанию они вырезаются**. Каждый тип отключается отдельным переключателем, если он всё-таки нужен.

Вырезание идёт **на обоих уровнях сразу**: и верхнеуровневые ключи `socks-port`, `port`, `mixed-port`, и записи `type: socks`, `type: http`, `type: mixed` в секции `listeners`. Одного места мало: выключенный в панели SOCKS иначе поднялся бы из второго.

Свой SOCKS5 и HTTP-прокси включаются там же, независимо друг от друга и **по умолчанию выключены**. У каждого свой порт и необязательные логин с паролем. Логин пустой — авторизации нет, и вход открыт всем в сети: он слушает все интерфейсы. Пустой список `users` пишется в конфиг явно, чтобы вход не унаследовал `authentication` из подписки и «без пароля» в панели означало именно это.

Порты проверяются до сохранения: они не должны совпадать друг с другом и с портами REDIR и TPROXY, иначе ядро откажется поднимать второй слушатель.
...
Все обычные настройки подписок находятся в веб-панели и сохраняются в `/etc/mihomo`. ENV нужны только для доступа к самой панели — всё остальное настраивается в ней и переживает перезапуск:
...
- **Обязательно задайте пароль панели Mihomo** в **Настройки → Панель Mihomo**. Порт `9090` — это
  RESTful API самого ядра, и он слушает все интерфейсы, чтобы панель открывалась с компьютера в
  той же сети. Пока secret пуст, любой в этой сети может без пароля переключать прокси, читать
  конфигурацию со всеми учётными данными подписки и подменять её. По той же причине не
  пробрасывайте `9090` в интернет.
